...
const shared_1 = require("@code-generator/shared");
class WebSocketService {
    ...
    async handleMessage(clientId, message) {
        try {
            const validatedMessage = shared_1.WebSocketMessageSchema.parse(message);
    ...
    async handleChatMessage(clientId, chatMessage) {
        try {
            ...
            if (this.isBuildRequest(chatMessage.content)) {
                ...
                // Generate PRD
                try {
                    const prd = await llm_service_1.llmService.generatePRD(chatMessage.content);
                    // Send PRD to client
                    this.sendToClient(clientId, {
                        type: 'prd',
                        payload: prd
                ...
                let fullResponse = '';
                await llm_service_1.llmService.streamChat(messages, (token) => {
    ...
    async handleStartBuild(clientId, payload) {
    ...
    async startBuildProcess(clientId, prompt, prd) {
        try {
            ...
            const projectInfo = prd || await llm_service_1.llmService.analyzeProject(prompt);
            ...
            const projectAnalysis = await llm_service_1.llmService.analyzeProject(prompt);
            this.sendToClient(clientId, {
                type: 'terminal',
                payload: {
            ...
            for (const file of projectAnalysis.files) {
                ...
                const generatedCode = await llm_service_1.llmService.generateCode(file.path, file.description, projectAnalysis);
                // Send file update
                this.sendToClient(clientId, {
                    type: 'fileUpdate',
                    payload: {
    ...
    generatePackageJson(projectName, additionalDeps) {
...
${projectInfo.features ? projectInfo.features.map((f) => `- ${f}`).join('\n') : '- Core features coming soon'}
...
${projectInfo.technicalRequirements ? projectInfo.technicalRequirements.map((r) => `- ${r}`).join('\n') : '- Next.js\n- TypeScript\n- Tailwind CSS'}
